...
class TTSService:
    ...
    def __init__(self, voice_vi: str = 'vi', voice_en: str = 'en'):
        ...
        self.logger.info(f"🔊 Initializing Google TTS Service (gTTS)...")
        self.logger.info(f"   Languages: Vietnamese (vi), English (en)")
        self.logger.info(f"   Status: FREE & UNLIMITED 🆓")
    ...
    async def synthesize(self, text: str, language: str = 'auto', 
        ...
        try:
            ...
            self.logger.info(f"🔊 Synthesizing with Google TTS...")
    ...
    async def test(self):
        ...
        # Test long text
        self.logger.info("📝 Test 5: Long text...")
        long_text = "Đây là một đoạn văn dài để kiểm tra khả năng xử lý của Google TTS. " * 3
        audio = await self.synthesize(long_text, "vi")
        if audio:
            self.logger.info(f"   ✅ Long text OK: {len(audio)} bytes")
        else:
            self.logger.error("   ❌ Long text FAILED")
